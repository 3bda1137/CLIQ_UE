    ...
    public class EditController : Controller
    {
        ...
        public IActionResult EditBio(EditBioAndUploadImageViewModel BioVM)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    string uploadsFolder = Path.Combine(webHostEnvironment.WebRootPath, "");
                    string uniqueFileName = Guid.NewGuid().ToString() + "_" + BioVM.Image.FileName;
                    string filePath = Path.Combine(uploadsFolder, uniqueFileName);
                    using (var fileStream = new FileStream(filePath, FileMode.Create))
                    {
                        BioVM.Image.CopyTo(fileStream);
                    }
                    // Set the ImagePath property of editBio to the relative path of the uploaded file
                    editBio.ImagePath = Path.Combine("uploads", uniqueFileName);
                }
